...
public class DateProcess
    {
       ...
       public static DateTime GetServerDate(string ConnectionString)
       {
           ...
           try
           {
               DataTable dt = DataProcess.GetData(ConnectionString, "select Getdate() as rd");
               if (dt.Rows.Count > 0)
               {
       ...
       public static string GetServerTime(string ConnectionString)
       {
           string tt ="00:00 AM";
           try
           {
               DataTable dt = DataProcess.GetData(ConnectionString, "SELECT CONVERT(VARCHAR(8),GETDATE(),108) AS HourMinuteSecond");
               if (dt.Rows.Count > 0)
               {
                   tt =dt.Rows[0]["HourMinuteSecond"].ToString();
               }
       ...
       public static DateTime GetDateByDay(DateTime inputDate,int prm)
       {
           DateTime dt = inputDate.AddDays(prm);  
           return dt;
       }      
       public static DateTime GetDateByMonth(DateTime inputDate, int prm)
       {
           DateTime dt = Convert.ToDateTime(inputDate.AddMonths(prm));
           return dt;
       }
       public static DateTime GetDateByYear(DateTime inputDate, int prm)
       {
           DateTime dt = Convert.ToDateTime(inputDate.AddYears(prm));
           return dt;
       }
       ...
       public static bool CheckValidDate(string dt)
       {
           try
           {
               DateTime dtc = Convert.ToDateTime(dt);
               return true;
           }
           catch
           {
