...
public partial class modules_HRMS_Approval_frmForeignTourExpenseApproval : Page 
{
    ...
    private void ClearControl()
    {
        ...
        lblArrivalFlight.Text = string.Empty;
        ...
        lblPlaceoftour.Text = string.Empty;
        lblCountry.Text = string.Empty;
        lblVendor.Text = string.Empty;
        lblDepartureFlight.Text = string.Empty;
    ...
    private void LoadAdvanceReceivedData(string transactionNo)
    {
        try
        {
            ...
            foreach (DataRow row in dtAdvanceReceived.Rows)
            {
                ...
                lblArrivalFlight.Text = row["arrivalFlight"].ToString();
                lblDuration.Text = row["durationTour"].ToString();
                ...
                lblPlaceoftour.Text = row["placeTour"].ToString();
                lblCountry.Text = row["countryTour"].ToString();
                lblVendor.Text = row["vendorTour"].ToString();
                lblDepartureFlight.Text = row["departureFlight"].ToString();
                lblPurpose.Text = row["purposeClaim"].ToString();
            }
    ...
    private void LoadExpensesBillDetails(string transactionNo)
    {
        ...
        _objConveyanceApplication.ActingPersonCode = Session["ActingPersonID"].ToString();
        _objConveyanceApplicationController = new ConveyanceApplicationController();
        var dtExpenseDetails = _objConveyanceApplicationController.ShowPendingApplicationDetails(_connectionString, _objConveyanceApplication);
        grdExpenseBillDetails.DataSource = null;
        grdExpenseBillDetails.DataBind();
        if (dtExpenseDetails.Rows.Count > 0)
        {
            grdExpenseBillDetails.DataSource = dtExpenseDetails;
            grdExpenseBillDetails.DataBind();
    ...
    private void ActionForApplicationApproval(int actionType)
    {
        try
        {
            foreach (GridViewRow rowNo in grdExpenseBillDetails.Rows)
            {
                _objConveyanceApplication = new ConveyanceApplication();
    ...
    protected void grdExpenseBillDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
